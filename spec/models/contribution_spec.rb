...
describe Contribution, type: :model do
  ...
  describe 'non-pull request validations' do
    ...
    it 'allows correcting historical date within original campaign year' do
      historical_contribution = build(
        :contribution,
        user: user,
        state: nil,
        created_at: Date.new(Tfpullrequests::Application.current_year - 1, 12, 30)
      )
      historical_contribution.save(validate: false)

      historical_contribution.created_at = Date.new(Tfpullrequests::Application.current_year - 1, 12, 20)
      historical_contribution.valid?
      expect(historical_contribution.errors[:created_at].count).to eq 0
    end
