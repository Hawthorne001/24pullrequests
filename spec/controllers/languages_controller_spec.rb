  ...
  describe 'GET show' do
    ...
    context 'when the language is valid' do
      ...
      context 'when there exists contributors for the language' do
        ...
        it 'deduplicates users when duplicate language skills exist' do
          create :skill, user: user1, language:

          show

          user_ids = assigns(:users).map(&:id)
          expect(user_ids.count(user1.id)).to eq(1)
        end
        ...
        it 'uses valid date range counts with aggregation filters applied' do
          ...
          expect(users[user1.id][:scoped_contributions_count].to_i).to eq(0)
          expect(users[user2.id][:scoped_contributions_count].to_i).to eq(1)
        end
      end
